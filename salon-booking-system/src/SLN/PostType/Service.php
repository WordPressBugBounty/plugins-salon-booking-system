...
class SLN_PostType_Service extends SLN_PostType_Abstract
{
    ...
    {
        if(!current_user_can('edit_sln_services')){
	...
	?>
		...
		<div class="col-sm-12 sln-inline-edit-service-fields">
			...
			<div class="col-sm-3">
			    <label>
				<span class="title"><?php esc_html_e( 'Slug' ); ?></span>
				<span class="input-text-wrap"><input type="text" name="sln_post_name" value="" /></span>
			    </label>
			</div>
			<div class="col-sm-3">
			    <label>
				<span class="title"><?php echo esc_html__( 'Price' ) . ' (' . $this->getPlugin()->getSettings()->getCurrencySymbol() . ')' ?></span>
				<span class="input-text-wrap"><input type="text" name="sln_service_price" value="" /></span>
			    </label>
			</div>
			<div class="col-sm-3">
			    <label>
				<span class="title title-inline"><?php esc_html_e( 'Units per hour' ); ?></span>
			...
			<div class="col-sm-3">
			    <label>
				<span class="title title-inline"><?php esc_html_e( 'Duration' ); ?></span>
				<?php SLN_Form::fieldTime('sln_service_duration'); ?>
			    </label>
			</div>
			<div class="col-sm-3">
			    <label>
				<span class="title title-inline"><?php esc_html_e( 'Secondary' ); ?></span>
				<?php SLN_Form::fieldCheckbox('sln_service_secondary') ?>
			    </label>
			</div>
			<div class="col-sm-3">
			    <label>
				<span class="title"><?php esc_html_e( 'Service Categories' ); ?></span>
    ...
    update_post_meta($post_id, '_' . $this->getPostType() . '_parallel_exec',
        isset($_POST['sln_service_parallel_exec']) && $_POST['sln_service_parallel_exec'] ? 1 : 0);
    ...
    private function categoryColumn($post_id){
        ...
        if ( is_array( $terms ) ) {
            ...
            foreach ( $terms as $t ) {
                ...
                $category_name = new SLN_Wrapper_ServiceCategory($t);

                $label = esc_html( sanitize_term_field( 'name', $t->getName(), $t->term_id, $taxonomy, 'display' ) );
            ...
            echo join( esc_html__( ', ' ), $term_links );
        } else {
