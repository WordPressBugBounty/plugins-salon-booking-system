...
class SLN_PostType_Booking extends SLN_PostType_Abstract
{
    ...
    public function posts_join ( $join, $wp_query ) {
        ...
        if ( is_admin() && 'edit.php' === $pagenow  && ! empty($_GET['post_type'] )  && $this->getPostType() === $_GET['post_type'] && ! empty( $_GET['s'] ) && ( isset($wp_query->query_vars['post_type'] )&& $wp_query->query_vars['post_type'] === $this->getPostType() ) ) {
            $join .= ' LEFT JOIN ' . $wpdb->postmeta . ' sln_mt1 ON ' . $wpdb->posts . '.ID = sln_mt1.post_id AND sln_mt1.meta_key = "_sln_booking_firstname" ';
            $join .= " LEFT JOIN {$wpdb->postmeta} sln_mt2 ON {$wpdb->posts}.ID = sln_mt2.post_id AND sln_mt2.meta_key = '_sln_booking_lastname' ";
            $join .= " LEFT JOIN {$wpdb->postmeta} sln_mt3 ON {$wpdb->posts}.ID = sln_mt3.post_id ";
        }
        return $join;
    }
    ...
    public function posts_where( $where ) {
        ...
        if ( is_admin() && 'edit.php' === $pagenow && ! empty($_GET['post_type'] ) && $this->getPostType() === $_GET['post_type'] && ! empty( $_GET['s'] ) ) {
            $where = preg_replace(
                ...
                " (" . $wpdb->posts . ".post_title LIKE $1) OR (sln_mt3.meta_value LIKE $1) OR (CONCAT(sln_mt1.meta_value, sln_mt2.meta_value) LIKE $1) ", $where );
        }
        return $where;
    }
        ...
        if ($post && $post->post_type === SLN_Plugin::POST_TYPE_BOOKING) {
            ...
            if(defined('SLN_VERSION_PAY') && SLN_VERSION_PAY){
                if (current_user_can('edit_' . SLN_Plugin::POST_TYPE_BOOKING. 's')) {
                    ...
                    if (class_exists('\SalonSOAP\Addon') && get_current_screen()->base == 'salon_page_salon-customers') {
                        $actions['soap_edit'] = '<a href="' . $this->getSOAPNotesActionPostLink($post->ID) . '" title="'
                            . esc_attr__("Edit SOAP notes", 'sln-soap')
                            . '">' . esc_html__('SOAP&nbsp;notes', 'sln-soap') . '</a>';
                    }
                }
            }
        }
    ...
    public function getSOAPNotesActionPostLink($id = 0)
    {
        return admin_url("post.php?action=edit&mode=sln_editor&post=$id");
    }
    ...
    public function manage_columns($columns)
    {
        $ret = array(
            ...
            'origin_source' => __('Origin', 'salon-booking-system'),
    ...
    public function manage_column($column, $post_id)
    {
        ...
        switch ($column) {
            ...
            case 'booking_status' :
                ...
                echo '<div style="width:14px !important; height:14px; border-radius:14px; border:2px solid '.$color.'; float:left; margin-top:2px;"></div> &nbsp;<span style="color:'.$color.'; font-weight:'.$weight.';">' . esc_html($status) . '</span>';
                break;
            ...
            case 'booking_price' :
                ...
                if (get_post_status($post_id) == SLN_Enum_BookingStatus::PAID && $deposit = get_post_meta(
                    ...
                    echo '(deposit '. esc_html($this->getPlugin()->format()->money($deposit)) .')';
                }
                break;
            ...
            case 'booking_attendant' :
                ...
                echo esc_html($obj->getAttendantsString());
                break;
            case 'booking_review' :
                $comments = get_comments("post_id=$post_id&type=sln_review");
                $comment = isset($comments[0]) ? $comments[0] : null;

                echo '<input type="hidden" name="sln-rating" value="'. esc_attr($obj->getRating()) .'">
                ...
                if (!empty($comment)) {
                    echo '<a href="'.esc_url(add_query_arg(array('p' => $post_id), admin_url('edit-comments.php'))).'#salon-review"
                            class="overflow-dots">'. esc_html($comment->comment_content) .'</a>';
                }
            ...
            case 'booking_reminder' :
                ...
                if (in_array($obj->getStatus(), $statuses)) {
                    ...
                    if ($this->getPlugin()->getSettings()->get('email_remind')) {
                        ...
                        if(!empty($email_remind) && $email_remind){
    ...
    public function bulkAdminFooter($isNew = false)
    {
        ...
        if ($post && $post->post_type == SLN_Plugin::POST_TYPE_BOOKING) {
            ...
            <script type="text/javascript">
                jQuery(function ($) {
                    $('#save-post').attr('value', '<?php echo esc_html__(
    ...
    public function restrict_manage_posts($post_type, $which = null) {
        ...
        if ($post_type === $this->getPostType() && $which === 'top') {
            ...
            $users = array();
            foreach($rows as $row) {
                $key = ($row->firstname ? $row->firstname : '') . '|' . ($row->lastname ? $row->lastname : '');
                $users[$key] = $row->firstname . ' ' . $row->lastname;
            }
            asort($users);
		...
		<select name="post_status" id="filter-by-post_status">
		    <option value=""><?php esc_html_e('All Statuses', 'salon-booking-system') ?></option>
	    ...
	    <?php endif ?>
            ...
            <select name="username" id="filter-by-username">
                <option value=""><?php esc_html_e('All users name', 'salon-booking-system') ?></option>
            ...
            <select name="service" id="filter-by-service">
                <option value=""><?php esc_html_e('All services', 'salon-booking-system') ?></option>
            ...
            <select name="attendant" id="filter-by-attendant">
                <option value=""><?php esc_html_e('All attendants', 'salon-booking-system') ?></option>
