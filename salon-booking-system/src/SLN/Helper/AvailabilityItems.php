...
class SLN_Helper_AvailabilityItems {
	 ...
	 * @return bool
	 */
	public function isValidDatetimeDuration( DateTime $date, DateTime $duration, SLN_Wrapper_ServiceInterface $service=null ) {
		$time     = Time::create( $date );
		$day      = Date::create( $date );
		$duration = Time::create( $duration );
		$ret      = $this->isValidTimeDuration( $day, $time, $duration, $service );
		if ( $time->isMidnight() ) {
			$yester = $day->getPrevDate();
			$ret    = $ret || $this->isValidTimeDuration( $yester, Time::create( '24:00' ), $duration, $service );
		}
		return $ret;
	}

	public function isValidDatetime( DateTime $date, SLN_Wrapper_ServiceInterface $service=null ) {
		$time = Time::create( $date );
		$day  = Date::create( $date );
		$ret  = $this->isValidTime( $day, $time, $service );
		if ( $time->isMidnight() ) {
			$yester = $day->getPrevDate();
			$ret    = $ret || $this->isValidTime( $yester, Time::create( '24:00' ), $service );
		}

		return $ret;
	}

	public function isValidDate( Date $day, SLN_Wrapper_ServiceInterface $service=null ) {
		$items = $this->getDateSubset( $day );
		foreach ( $items as $av ) {
			if ( $av->isValidDate( $day, $service ) ) {
				return true;
			}
		}
	...
	private function isValidTime( Date $date, Time $time, SLN_Wrapper_ServiceInterface $service=null ) {
		if ( $this->getOffset() ) {
	 ...
	 * @return bool
	 */
	private function isValidTimeDuration( Date $date, Time $time, Time $duration, SLN_Wrapper_ServiceInterface $service=null ) {
		$interval = new TimeInterval( $time, $time->add( $duration ) );
		return $this->isValidTimeInterval( $date, $interval, $service );
	}

	private function isValidTimeInterval( Date $date, TimeInterval $interval, SLN_Wrapper_ServiceInterface $service=null ) {
		if ( $interval->isOvernight() ) {
			...
			$items = $this->getDateSubset( $date );
			foreach ( $items as $av ) {
				if ( $av->isValidDate( $date, $service ) && $av->isValidTimeInterval( $interval ) ) {
					return true;
				}
			}
