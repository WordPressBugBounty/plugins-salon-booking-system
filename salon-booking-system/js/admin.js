...
jQuery(function ($) {
    ...
    $("#_sln_attendant_email").on("change", function () {
        ...
        $('[name="_sln_attendant_limit_staff_member_to_assigned_bookings_only"]').prop("checked", false);
        $('[name="_sln_attendant_limit_staff_member_to_backend_calendar_only"]').prop("checked", false);
        if (+user_id) {
    ...
    function sln_select2MultiSearch() {
        $('.sln-select--multiple--search select').each(function () {
            jQuery(this).on('select2:closing', function (e) {
                if (jQuery(this).parent().find('.select2-selection__rendered .select2-selection__choice').length) {
                    jQuery(this).closest('.sln-select--inwrapper').removeClass('has_no_choices');
                } else {
                    jQuery(this).closest('.sln-select--inwrapper').addClass('has_no_choices');
                }
            });
        });
    }
