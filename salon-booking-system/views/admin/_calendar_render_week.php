			...
			<div class="<?php echo $cell_class; ?> <?php echo $holiday; ?>" >
				<a href="#" class="calbar week-calbar" data-placement="<?php echo $weekStart->format('N') > 3 ? 'left' : 'top';?>" data-toggle="tooltip" data-html="true" data-day="<?php echo $weekStart->format('Y-m-d'); ?>" data-original-title='<?php echo $stat["text"];?>'>
                    <?php if(isset($stat['free'])): ?>
                        <span class="busy" style="width: <?php echo $stat['busy']; ?>%"></span>
                        <span class="free" style="width: <?php echo $stat['free']; ?>%"></span>
                    <?php endif;?>
                </a>
				<?php echo $wp_locale->get_weekday($weekStart->format('w')); ?><br>
				<small><span data-cal-date="<?php echo $weekStart->format('Y-m-d');?>" data-cal-view="day"><?php echo $weekStart->format('j'). ' '. $wp_locale->get_month_abbrev($wp_locale->get_month($weekStart->format('m'))); ?></span></small>
			</div><?php
